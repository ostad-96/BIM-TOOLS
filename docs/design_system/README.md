...
- No purple/blue gradients — gradients are always brand teal to darker brand teal, never multi-hue.
- No emoji in UI copy.
...
|---|---|---|
| Hub, Login, Sitzungsprotokolle, Admin, Issuemanagement, Dokumentenmanagement, BIM-Koordination | **Corporate refresh** (`body.corp`) | Browser — desktop & mobile |
...
- **Pick groups**: dashed `#e5e7eb` border in waiting state to solid `#003B4D` + tint `#f0f7fa` while picking to solid `#4a5d6b` + `#f0f5f7` when done. Inline pill on the right shows the state in uppercase (`Warten` / `Auswahlen...` / `Fertig`).
...
- No `border-radius: 14 px` on panels — VDC panels are 8 px, reflecting the more clinical engineering tone.
...
- Icons inherit color from `currentColor`. Module-tile icons sit inside a 38x38 `--c-brand-soft` rounded-square; the icon itself is `--c-brand`. Admin module overrides to `#7c3aed` on `#ede9fe`.

Because there is **no shared icon file** in the codebase, when extending the system you should **either** (a) copy an existing SVG from a page that already has the same concept, **or** (b) drop in the matching Lucide icon at stroke-width `1.8` — it is the closest visual match. **Flag any substitution to the user.**
...
- **Iconography** — no icon library is used in production; each page reinvents the SVGs it needs. For new icons not yet drawn in the product, prefer Lucide as the substitute and flag the choice.
...
3. **One brand color, one accent color.** If a design needs a third color, push back — the system gets its identity from restraint.
